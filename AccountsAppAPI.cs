...
public class AccountsAppAPI : DataBaseConnection
{
    ...
    [WebMethod]
    public DataSet GetAccountGroup(string SKey, int InstId)
    {
        param = new SqlParameter[1];
        param[0] = new SqlParameter("@InstId", InstId);
    ...
    /// <returns></returns>
    [WebMethod]
    public DataSet AccountLedgerForTransaction(string SKey, int InstId, int FinancialYearId, int DepartmentId, int GroupId)
    {
        try
        {
            param = new SqlParameter[4];
    ...
    public int TransactionMasterInsert(int InstId, TransactionMaster transactionMaster, List<TransactionDetail> transactionDetail, int OldTransactionMasterId)
    {
        try
        {
            ...
            TransactionDetailsBackup(skey, jsonData);//Added for backup changes
            db.SaveChanges();
    ...
    [WebMethod]
    public bool InsertAccountGroup(string SKey, AccountGroupMaster Ac, int InstId, bool IsModify)
    {
        try
        {
            if (IsModify)
    ...
    [WebMethod]
    public bool InsertAccountLedger(string SKey, AccountLedgerMaster alm, AccountLedger AL, AccountLedgerOpeningBalance ob, bool IsModify, int OldForInstId)
    {
        try
        {
            if (IsModify)
    ...
    public DataSet TransactionDetailsBackup(string SKey, string Jsondata)
    {
        try
        {
            param = new SqlParameter[1];
